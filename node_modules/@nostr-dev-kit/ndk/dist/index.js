...
async function generateContentTags(content, tags = []) {
  ...
  content = content.replace(tagRegex, (tag) => {
    try {
      ...
      const { type, data } = import_nostr_tools.nip19.decode(entity);
      let t;
      switch (type) {
        case "npub":
          t = ["p", data];
...
function encode(maxRelayCount = DEFAULT_RELAY_COUNT) {
  ...
  if (this.isParamReplaceable()) {
    return import_nostr_tools2.nip19.naddrEncode({
      kind: this.kind,
      pubkey: this.pubkey,
    ...
    return import_nostr_tools2.nip19.neventEncode({
      id: this.tagId(),
      relays,
      author: this.pubkey
    });
  } else {
    return import_nostr_tools2.nip19.noteEncode(this.tagId());
  }
}
...
function filterForEventsTaggingId(id) {
  try {
    const decoded = import_nostr_tools3.nip19.decode(id);
    switch (decoded.type) {
      case "naddr":
        return {
          "#a": [
...
function filterFromId(id) {
  ...
  if (id.match(BECH32_REGEX)) {
    try {
      decoded = import_nostr_tools3.nip19.decode(id);
      switch (decoded.type) {
        case "nevent": {
...
function relaysFromBech32(bech322, ndk) {
  try {
    const decoded = import_nostr_tools3.nip19.decode(bech322);
  ...
  async getArticle() {
    ...
    switch (articleTag[0]) {
      case "a":
        ...
        taggedBech32 = import_nostr_tools4.nip19.naddrEncode({ kind: parseInt(kind), pubkey, identifier });
        break;
      case "e":
        taggedBech32 = import_nostr_tools4.nip19.noteEncode(articleTag[1]);
        break;
      case "r":
...
var NDKNutzap = class _NDKNutzap extends NDKEvent {
  ...
  constructor(ndk, event) {
    ...
    this.debug = ndk?.debug.extend("nutzap") ?? (0, import_debug3.default)("ndk:nutzap");
...
var NDKUser = class _NDKUser {
  ...
  get npub() {
    if (!this._npub) {
      ...
      this._npub = import_nostr_tools5.nip19.npubEncode(this.pubkey);
    }
    return this._npub;
  }
  get nprofile() {
    const relays = this.profileEvent?.onRelays?.map((r) => r.url);
    return import_nostr_tools5.nip19.nprofileEncode({
      pubkey: this.pubkey,
      relays
    });
  }
  ...
  get pubkey() {
    if (!this._pubkey) {
      ...
      this._pubkey = import_nostr_tools5.nip19.decode(this.npub).data;
    }
    return this._pubkey;
  }
  /**
...
function disconnect(pool, debug8) {
  debug8 ??= (0, import_debug6.default)("ndk:relay:auth-policies:disconnect");
  return async (relay) => {
...
function signIn({ ndk, signer, debug: debug8 } = {}) {
  debug8 ??= (0, import_debug6.default)("ndk:auth-policies:signIn");
...
var NDKNip07Signer = class {
  ...
  constructor(waitTimeout = 1e3) {
    this.debug = (0, import_debug7.default)("ndk:nip07");
...
var NDKPrivateKeySigner = class _NDKPrivateKeySigner {
    ...
    if (privateKey) {
      if (typeof privateKey === "string") {
        if (privateKey.startsWith("nsec1")) {
          const { type, data } = import_nostr_tools7.nip19.decode(privateKey);
      ...
      if (this._privateKey) {
        this._user = new NDKUser({
          pubkey: (0, import_nostr_tools6.getPublicKey)(this._privateKey)
        });
      }
    }
  }
  ...
  static generate() {
    const privateKey = (0, import_nostr_tools6.generateSecretKey)();
  ...
  async sign(event) {
    ...
    return (0, import_nostr_tools6.finalizeEvent)(event, this._privateKey).sig;
  }
  ...
  async encrypt(recipient, value, scheme) {
    ...
    const recipientHexPubKey = recipient.pubkey;
    if (scheme == "nip44") {
      let conversationKey = import_nostr_tools6.nip44.v2.utils.getConversationKey(this._privateKey, recipientHexPubKey);
      return await import_nostr_tools6.nip44.v2.encrypt(value, conversationKey);
    }
    return await import_nostr_tools6.nip04.encrypt(this._privateKey, recipientHexPubKey, value);
  }
  async decrypt(sender, value, scheme) {
    ...
    const senderHexPubKey = sender.pubkey;
    if (scheme == "nip44") {
      let conversationKey = import_nostr_tools6.nip44.v2.utils.getConversationKey(this._privateKey, senderHexPubKey);
      return await import_nostr_tools6.nip44.v2.decrypt(value, conversationKey);
    }
    return await import_nostr_tools6.nip04.decrypt(this._privateKey, senderHexPubKey, value);
  }
};
...
var NDKNip46Backend = class {
  ...
  constructor(ndk, privateKeyOrSigner, permitCallback, relayUrls) {
    ...
    if (privateKeyOrSigner instanceof Uint8Array) {
      ...
      this.signer = new NDKPrivateKeySigner((0, import_utils4.hexToBytes)(privateKeyOrSigner));
...
var NDKSubscriptionManager = class {
  ...
  dispatchEvent(event, relay, optimisticPublish = false) {
    ...
    for (const sub of subscriptions) {
      if ((0, import_nostr_tools8.matchFilters)(sub.filters, event)) {
...
async function generateZapRequest(target, ndk, data, pubkey, amount, relays, comment, tags, signer) {
  ...
  const zapRequest = import_nostr_tools9.nip57.makeZapRequest({
    profile: pubkey,
...
var d2 = (0, import_debug10.default)("ndk:zapper:ln");
